...
public class InnerSegmentAggregationSingleValueQueriesTest extends BaseSingleValueQueriesTest {
  ...
  public void testSingleColumnDistinct() {
    ...
    Assert.assertEquals(distinctTable.getProjectedColumnNames().length, 1);
    Assert.assertEquals(distinctTable.getProjectedColumnTypes().length, 1);
    Assert.assertEquals(distinctTable.getProjectedColumnNames()[0], "column1");
    Assert.assertEquals(distinctTable.getProjectedColumnTypes()[0], FieldSpec.DataType.INT);
  ...
  public void testMultiColumnDistinct() {
    ...
    Assert.assertEquals(distinctTable.getProjectedColumnNames().length, 2);
    Assert.assertEquals(distinctTable.getProjectedColumnTypes().length, 2);
    Assert.assertEquals(distinctTable.getProjectedColumnNames()[0], "column1");
    Assert.assertEquals(distinctTable.getProjectedColumnNames()[1], "column3");
    Assert.assertEquals(distinctTable.getProjectedColumnTypes()[0], FieldSpec.DataType.INT);
    Assert.assertEquals(distinctTable.getProjectedColumnTypes()[1], FieldSpec.DataType.INT);
