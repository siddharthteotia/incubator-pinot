...
public class DefaultAggregationExecutor implements AggregationExecutor {
  ...
  public void aggregate(@Nonnull TransformBlock transformBlock) {
    ...
    for (int i = 0; i < _numFunctions; i++) {
      ...
      final AggregationResultHolder resultHolder = _resultHolders[i];
      if (function.getType() == AggregationFunctionType.COUNT) {
        ...
        final BlockValSet[] blockValSets = new BlockValSet[_expressions.length];
        for (int j = 0; j < _expressions.length; j++) {
          blockValSets[j] = transformBlock.getBlockValueSet(_expressions[j]);
        }
        final DistinctAggregationFunction distinctFunction = (DistinctAggregationFunction)function;
  ...
  public List<Object> getResult() {
    ...
    //DistinctAggregationFunction function = (DistinctAggregationFunction)_functions[0];
    //function.dumpTime();
