...
public class UpdateAliasNode implements PlanTreeNode{
  ...
  public UpdateAliasNode(QueryContext queryContext, List<String> originalCols, List<String> aliasCols, TableConfig tableConfig) {
    assert (queryContext.getAliasList() != null && !queryContext.getAliasList().isEmpty()
        && !originalCols.isEmpty() && !aliasCols.isEmpty());
