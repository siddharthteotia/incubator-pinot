 ...
 * characteristics similar to a given source dataset. The tool can be
 * used in situations where actual source data isn't allowed
 * to be used for the purpose of testing (regression,
 * performance, functional, evaluation of other OLAP systems etc).
 *
 * The current implementation takes a set of Pinot segments
 * as input source dataset and generates equal number of Avro
 * files.
 *
 * Implementation notes are mentioned further in the source code.
 *
 * Limitations:
 ...
 *
 */
public class PinotDataGenerator {
   ...
   * on the actual data yield roughly same results on generated data
   *
    ...
    private int isLessThan(Object o1, Object o2) {
      switch (_dataType) {
        ...
        case LONG:
          return ((Long) o1).compareTo((Long)o2);
        default:
          throw new IllegalStateException("illegal type in binary search");
      }
    }
  }
  ...
  public void buildGlobalDictionaries() throws Exception {
    ...
    System.out.println("Finished building global dictionaries. Time taken: " + _timeToBuildDictionaries.elapsed(TimeUnit.SECONDS) + "secs");
  ...
  private void readDictionariesFromSegment(String segmentDirectory) throws Exception {
    ...
    for (Map.Entry<String, ColumnMetadata> entry : columnMetadataMap.entrySet()) {
      ...
      if (dictionary != null) {
        ...
        throw new UnsupportedOperationException("Data generator currently does not support columns without dictionary");
      }
    }
  }
  ...
  private void writeGlobalDictionariesAndColumnMapping() throws Exception {
    ...
    stopwatch.stop();
    System.out.println("Finished writing global dictionaries and column name mapping to disk. Time taken: " + stopwatch.elapsed(TimeUnit.SECONDS) + "secs");
  }
   ...
   *
   * Eventually this tool might be run as a series of commands.
   */
  ...
  public void generateAvroFiles()
    ...
    for (int file = 0 ; file < _numFilesToGenerate; file++) {
      ...
      System.out.println("Using segment " + pathToSegment + " to generate Avro file");
      ...
      try (PinotSegmentRecordReader recordReader = new PinotSegmentRecordReader(segmentIndexDir, _pinotSchema, null);
        ...
        totalRows += rows;
        System.out.println("Generated Avro File " + pathToAvroFile +  " with " + rows + "  rows");
      }
    }

    _timeToGenerateAvroFiles.stop();
    System.out.println("Finished generating " + totalRows + " rows across " + _numFilesToGenerate + " Avro files. Time taken: " + _timeToGenerateAvroFiles.elapsed(TimeUnit.SECONDS) + "secs");
  }
    ...
    for (FieldSpec fieldSpec : pinotSchema.getAllFieldSpecs()) {
      ...
      switch (dataType) {
        ...
        default:
          ...
          throw new RuntimeException("Data generator does not support type: " + dataType);
      }
    }
  ...
  public static class QueryGenerator {
    ...
    public void generateQueries() throws Exception {
      ...
      System.out.println("Finished generating " + count + " queries. Time taken: " + _generateQueryWatch.elapsed(TimeUnit.SECONDS) + "secs");
      out.flush();
    }
    ...
    public void generateQuery(String origQuery, PrintWriter out) {
      ...
      //System.out.println("Generated query: " + genQuery.toString());
    }
    ...
    private void loadGlobalDictionariesAndColumnMapping() throws Exception {
      ...
      System.out.println("Finished reading column name mapping");
      ...
      System.out.println("Finished reading column dictionaries");
    }
  ...
  /**
   * The purpose of this is to only test. It is very rudimentary
   * and eventually this tool might be used from command line
   * with individual commands for each of the steps.
   *
   * @param args
   * @throws Exception
   */
  public static void main(String[] args) throws Exception {
    String segmentDir = "/home/steotia/kusto/snapActivity/segmentDir";
    String outputDir = "/home/steotia/kusto/snapActivity/outputDir";
    String queryDir = "/home/steotia/kusto/snapActivity/queryDir/broker_43294";
    String queryFileName = "queries.log";
    String anonymousTableName = "MyTable";

    // STEP 1: use query file to extract filter columns
    Set<String> filterColumns = extractFilterColumns(queryDir, queryFileName);

    // STEP 2: user identifies the time/date columns we want to retain the
    // original data for
    Set<String> timeColumns = identifyTimeColumns();

    // STEP 3: manually enter total cardinality for extracted filter columns
    Map<String, Integer> globalDictColumns = getTotalCardinalityForFilterColumns(filterColumns, timeColumns);

    // STEP 4 and 5: build global dictionaries and generate Avro files
    buildGlobalDictionariesAndGenerateData(segmentDir, outputDir, anonymousTableName + "_", globalDictColumns, timeColumns);

    // STEP 6: generate queries
    generateQueries(outputDir, queryDir, queryFileName, anonymousTableName, filterColumns, timeColumns);
  }

  private static Set<String> extractFilterColumns(
      String queryDir,
      String queryFileName) throws Exception {
    Set<String> filterColumns = PinotDataGenerator.FilterColumnExtractor.extractColumnsUsedInFilter(queryDir, queryFileName);
    System.out.print("Columns participating in filters:");
  ...
  private static Set<String> identifyTimeColumns() {
    Set<String> timeColumns = new HashSet<>();
    timeColumns.add("timestamp");
    timeColumns.add("date");
    timeColumns.add("year_month_day_hour");
    System.out.print("\nTime columns:");
  ...
  private static Map<String, Integer> getTotalCardinalityForFilterColumns(Set<String> filterColumns, Set<String> timeColumns) {
    // manual step
    for (String column: timeColumns) {
      // sometime the predicates can be on the time columns
      // and thus these columns will be part of user supplied
      // global dictionary column set.
      // But since the time column values are retained, we
      // don't need to build global dictionary for them,
      // so remove them from global dictionary column set
      // Note: user can also do this sanitization
      filterColumns.remove(column);
    }

    System.out.print("\nColumns to build global dictionary for:");

    Map<String, Integer> globalDictColumns = new HashMap<>();
    for (String column : filterColumns) {
      System.out.println(column);
      if (column.equalsIgnoreCase("contract_id")) {
        globalDictColumns.put(column, 4095);
      } else {
        globalDictColumns.put(column, 0);
      }
    }
    System.out.println();
  ...
  private static void buildGlobalDictionariesAndGenerateData(
      String segmentDir,
      String outputDir,
      String filePrefix,
      Map<String, Integer> globalDictColumns,
      Set<String> timeColumns) throws Exception {
    PinotDataGenerator pinotDataGenerator = new PinotDataGenerator(
        segmentDir,
        outputDir,
        filePrefix,
        globalDictColumns,
        timeColumns);
    // right now these steps are combined
  ...
  private static void generateQueries(
      ...
      String anonymousTableName,
      Set<String> filterColumns,
      Set<String> timeColumns) throws Exception {
    // it needs outputDir since it will load the global dictionaries and column name mappings
    // used for generating queries
    // generated queries will be written in queryDir
    PinotDataGenerator.QueryGenerator queryGenerator = new QueryGenerator(
        outputDir, queryDir, queryFileName, anonymousTableName, filterColumns, timeColumns);
